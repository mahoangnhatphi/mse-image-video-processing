...
magnitude = sqrt(filteredImg1.^2 + filteredImg2.^2);
% logMagnitude = log(magnitude) + 1;
% logMagnitude = logMagnitude / max(logMagnitude(:));
% Apply magnitude thresholds
threshold150 = magnitude > 150;
threshold300 = magnitude > 300;
threshold900 = magnitude > 900;
...
figure;
subplot(2, 2, 1);
imshow(adjustedMagnitude);
title('Adjusted Magnitude');

subplot(2, 2, 2);
imshow(adjustedThreshold150);
title('Adjusted Magnitude > 150');

subplot(2, 2, 3);
imshow(adjustedThreshold300);
title('Adjusted Magnitude > 300');

subplot(2, 2, 4);
imshow(adjustedThreshold900);
title('Adjusted Magnitude > 900');